...
- `id` (PK), `uniplus_id` (unique, not null — prefixado por origem, ex. `dav-123`/`nf-456`, para evitar colisão entre os dois espaços de ID do Uniplus), `source_type` (enum: `dav`, `nota_fiscal`, not null — permite segmentar relatórios por origem; ver regra de deduplicação DAV × Nota Fiscal em `docs/uniplus-schema/02-regras-negocio-uniplus.md`), `customer_id` (FK → customers, nullable até vínculo), `seller_id` (FK → sellers, nullable), `sale_date` (timestamp, not null), `total_amount` (numeric), `synced_at`, `created_at`.
- Índices: `sale_date` (usado em atribuição de campanha e relatórios), `customer_id`, `seller_id`, `source_type`.
...
- ~~**Mapeamento do schema do banco de dados do Uniplus (e, se aplicável, do Integrar):** este FSD assume a existência de tabelas-espelho no CRM Live (`customers`, `sales`, `sale_items`, `products`, `stock_snapshots`) alimentadas pela sincronização somente-leitura com o Uniplus, mas não mapeia as tabelas e colunas reais do banco do Uniplus às colunas dessas tabelas-espelho (seção 11.2).~~ **Resolvida em 08–10/08/2026.** O responsável levantou o schema real do banco `unico` (Uniplus) via ferramenta somente leitura (`backend/app/integrations/uniplus/schema-explorer.js`) e confirmou 12 tabelas de origem (`dav`, `davitem`, `entidade`, `filial`, `hierarquia`, `item`, `notafiscal`, `notafiscalitem`, `operacao_nfce_view`, `produto`, `produtoean`, `saldoestoque`) e as colunas necessárias de cada uma — documentado em `docs/uniplus-schema/01` a `04`. Junto com o mapeamento, duas decisões de modelo de dados foram tomadas: (1) `sales` ganha a coluna `source_type` (`dav`/`nota_fiscal`) para permitir segmentar relatórios por origem, já refletida em §11.2; (2) a validação de WhatsApp na sincronização usa checagem ativa contra o próprio WhatsApp (via nova capacidade na camada de mensageria da Fase 6), testando `whatsapp`/`celular`/`telefone` de `entidade` nessa ordem, em vez da coluna `entidade.tipowhatsapp` (descartada). As colunas da tabela `item` foram confirmadas em 10/08/2026 (tem papel próprio, com colunas que referenciam tanto `dav` quanto `notafiscal` — ver observação em `docs/uniplus-schema/04-colunas-confirmadas.md`), fechando o mapeamento das 12 tabelas de origem. A implementação do job de sincronização (seção 6.9/13.10) ainda não foi iniciada.
